/**
 * Minimal Vanilla JS QR Code Generator
 * Generates an SVG or Canvas QR code directly in the browser with 0 external network requests.
 */
var QRCode = (function () {
    ...
    QRCode.prototype.makeCode = function (text) {
        var oQRCode = new QRCodeModel(this.options.typeNumber, this.options.correctLevel);
        oQRCode.addData(text);
        oQRCode.make();

        var nCount = oQRCode.getModuleCount();
        var nWidth = this.options.width;
        var nHeight = this.options.height;
        ...
        for (var row = 0; row < nCount; row++) {
            for (var col = 0; col < nCount; col++) {
                if (oQRCode.isDark(row, col)) {
                    ctx.fillRect(
                        Math.round(col * (nWidth / nCount)),
                        Math.round(row * (nHeight / nCount)),
                        Math.ceil(nWidth / nCount),
                        Math.ceil(nHeight / nCount)
                    );
                }
            }
        }
